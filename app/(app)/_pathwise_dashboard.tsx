...
function EnrolledRoadmapCard({ roadmapMeta, index, progressData, colors, styles, onPress }: any) {
  ...
  const progressPercent = React.useMemo(() => {
    // Use roadmapMeta.id as key — this is what the detail screen saves progress with
    const roadmapId = roadmapMeta.id;
    ...
    const roadmapForCalc = fullRoadmap || roadmapMeta;
    roadmapForCalc.modules?.forEach((m: any) => {
      m.topics?.forEach((t: any) => {
        // Mirror EXACT logic from [id].tsx line 200:
        // use objectives if non-empty, else problems if non-empty, else the topic itself
        const items =
          (t.objectives?.length > 0 ? t.objectives : null) ||
          (t.problems?.length > 0 ? t.problems : null) ||
          [t];
  ...
  }, [fullRoadmap, roadmapMeta, progressData]);
